...
if(!isset($_SESSION['ID'])){
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($errmsg == 1) {
            ...
            try {
                $sql = "INSERT INTO tghousandcontent (classabbr, classlong, description, inputdate, userID) 
                    VALUES (:classabbr, :classlong, :description, :inputdate, :userID) ";
                $stmt = $pdo->prepare($sql);
                $stmt->bindValue(':classabbr', $formdata['classabbr']);
...
//display form if Show Form Flag is true
    if ($showform == 1) {
        ?>
        <form name="addclass" id="addclass" method="post" action="contentcreate.php">
            <table>
                <tr>
                    <th><label for="classabbr">Abbreviated Class Name:</label></th>
                    <td><input name="classabbr" id="classabbr" type="text" size="20" placeholder="Required Abbreviation"
                               value="<?php if (isset($formdata['classabbr'])) {
                                   echo $formdata['classabbr'];
                               } ?>"/>
                        <span class="error">*<?php if (isset($errabbr)) {
                                echo $errabbr;
                            } ?></span></td>
                </tr>
                <tr>
                    <th><label for="classlong">Full Class Name:</label></th>
                    <td><input type="text" name="classlong" id="classlong" size="30" placeholder="Required Full Name"
                               value="<?php if (isset($formdata['classlong'])) {
                                   echo $formdata['classlong'];
                               } ?>"/>
                        <span class="error">*<?php if (isset($errlong)) {
                                echo $errlong;
                            } ?></span></td>
                </tr>
                <tr>
                    <th><label for="description">Description:</label></th>
                    <td><span class="error">* <?php if (isset($errdescription)) {
                                echo $errdescription;
                            } ?></span>
                        <textarea name="description" id="description"
                                  placeholder="Required Description"><?php if (isset($formdata['bio'])) {
                                echo $formdata['bio'];
                            } ?></textarea>
                    </td>
                </tr>
                <tr>
                    ...
                    <td><input type="hidden" name="userID" id="userID" value="<?php echo $_SESSION['ID'] ?>"/>
